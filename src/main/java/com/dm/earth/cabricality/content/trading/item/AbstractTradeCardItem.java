...
public abstract class AbstractTradeCardItem extends Item {
	...
	public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
		...
		if ((result.getResult() == ActionResult.SUCCESS || result.getResult() == ActionResult.CONSUME) && isPlayerTradeAble(player, tradeCostFoodLevel)) {
			reducePlayerFood(player, tradeCostFoodLevel);
			player.getItemCooldownManager().set(this, tradeCostFoodLevel * 25);
			return new TypedActionResult<>(world.isClient() ? ActionResult.SUCCESS : ActionResult.CONSUME, result.getValue());
		} else return TypedActionResult.fail(player.getStackInHand(hand));
	}

	private static boolean isPlayerTradeAble(PlayerEntity player, int level) {
		return player.getHungerManager().getFoodLevel() >= level || player.isCreative() || player.getHungerManager().getFoodLevel() >= level;
	}

	private static void reducePlayerFood(PlayerEntity player, int cost) {
		if (player.getWorld().isClient()) return;
		if (player.getHungerManager().getSaturationLevel() >= cost * 3) player.getHungerManager().add(0, -3 * cost);
		else if (player.getHungerManager().getFoodLevel() >= cost) player.getHungerManager().add(-1 * cost, 0.0F);
	}
}
